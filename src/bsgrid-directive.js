 ...
 */
angular.module('bsgrid', ['angular-loading-bar']).directive('bsGrid', function () {
  return {
    scope: {
      options: '='
    },
        ...
        if (data) {
          ...
          if (angular.isDefined(data.columns)) {
            ...
            _.forEach(data.columns, function (c, i) {
              ...
              if (index >= 0) {
                if (i !== index) {
                  var tmp = $scope.options.columns[index];
                  $scope.options.columns[index] = $scope.options.columns[i];
                  $scope.options.columns[i] = tmp;
                }
                $scope.options.columns[i].visible = c.visible;
                $scope.options.columns[i].sorted = c.sorted;
                if (angular.isDefined(c.reverse)) {
      ...
      $scope.GetRowsForExport = function (exportType, fieldNames, csvData) {
        ...
        _.each(itemsToExport, function (itemToExport) {
          ...
          _.each(fieldNames, function (fieldName) {
            ...
            if (angular.isDefined(itemToExport[fieldName])) {
              curCellRaw = itemToExport[fieldName];
            }
            else if (angular.isDefined(itemToExport.search) && angular.isDefined(itemToExport.search[fieldName])) {
